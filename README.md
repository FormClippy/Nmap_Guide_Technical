...
| คำสั่ง                 | คำอธิบายแบบมีหลักการ                                      | ตัวอย่าง                                    |
| ---------------------- | --------------------------------------------------------- | ------------------------------------------- |
...
| คำสั่ง   | คำอธิบายแบบมีหลักการ                                                                     | ตัวอย่าง                  |
| -------- | ---------------------------------------------------------------------------------------- | ------------------------- |
| `-sn`    | Ping Scan: ใช้สำหรับตรวจสอบว่า host ปลายทาง online หรือไม่ โดยไม่สแกนพอร์ต               | `nmap -sn 192.168.1.0/24` |
...
| `-PE`    | ใช้ ICMP Echo Request (เช่นเดียวกับคำสั่ง ping)                                          | `nmap -PE 192.168.1.5`    |
...
| คำสั่ง            | คำอธิบายแบบมีหลักการ                                                                                     | ตัวอย่าง               |
| ----------------- | -------------------------------------------------------------------------------------------------------- | ---------------------- |
| `-sS`             | TCP SYN Scan: ใช้เทคนิคส่ง SYN โดยไม่ทำการจับมือ 3 ขั้นตอน (half-open) จึง stealth และเร็ว               | `nmap -sS 192.168.1.5` |
| `-sT`             | TCP Connect Scan: ใช้ฟังก์ชัน system call `connect()` ทำ handshake ครบเหมือน browser ทั่วไป              | `nmap -sT 192.168.1.5` |
...
| `-p 1-65535` | สแกนทุก port ที่เป็นไปได้ใน TCP/UDP stack | `nmap -p 1-65535 192.168.1.5` |
| `-F` | Fast Scan: สแกนเฉพาะ port ที่พบบ่อย (~100 พอร์ต) เพื่อความเร็ว | `nmap -F 192.168.1.5` |
| `--top-ports 100` | สแกนเฉพาะ 100 พอร์ตยอดนิยมจากฐานข้อมูลของ Nmap | `nmap --top-ports 100 192.168.1.5` |

---

## ⚙️ Service / Version Detection (การตรวจสอบบริการและเวอร์ชัน)

| คำสั่ง | คำอธิบายแบบมีหลักการ | ตัวอย่าง |
|--------|------------------------|-----------|
| `-sV` | ตรวจสอบบริการที่รันอยู่บนพอร์ตและระบุเวอร์ชันโดยใช้ probe และ pattern matching | `nmap -sV 192.168.1.5` |
| `--version-intensity` | ปรับระดับความละเอียดของการ probe (0 = เร็ว, 9 = ครอบคลุมมากสุด) | `nmap -sV --version-intensity 9 192.168.1.5` |
...
| คำสั่ง | คำอธิบายแบบมีหลักการ | ตัวอย่าง |
|--------|------------------------|-----------|
| `-sC` | ใช้ชุดสคริปต์ default ที่ Nmap แนะนำซึ่งครอบคลุม service detection, vuln scan | `nmap -sC 192.168.1.5` |
| `--script=vuln` | ใช้สคริปต์ที่เน้นการตรวจหาช่องโหว่ที่รู้จักในบริการที่รันอยู่ | `nmap --script=vuln 192.168.1.5` |
| `--script=http-enum` | ใช้ตรวจสอบ endpoint และ directory ใน web service | `nmap --script=http-enum 192.168.1.5` |

---

## 💻 OS Detection (การระบุระบบปฏิบัติการของเป้าหมาย)

| คำสั่ง | คำอธิบายแบบมีหลักการ | ตัวอย่าง |
|--------|------------------------|-----------|
| `-O` | ใช้ TCP/IP fingerprinting เพื่อประเมินว่าเป้าหมายใช้ OS ใด | `nmap -O 192.168.1.5` |
| `--osscan-guess` | เปิดโหมด guessing เมื่อ fingerprint ไม่ตรงชัดเจน | `nmap -O --osscan-guess 192.168.1.5` |
| `-A` | รวมการทำ OS detection, version scan, script scan และ traceroute | `nmap -A 192.168.1.5` |
...
| คำสั่ง | คำอธิบายแบบมีหลักการ | ตัวอย่าง |
|--------|------------------------|-----------|
| `-T0` ถึง `-T5` | ปรับ profile ความเร็วในการสแกน (T0 = paranoid, T5 = aggressive) | `nmap -T4 192.168.1.5` |
| `--min-rate` | กำหนด minimum packets/second | `nmap --min-rate 1000 192.168.1.5` |
| `--max-rate` | กำหนด upper limit ของ rate ในการยิง packet | `nmap --max-rate 500 192.168.1.5` |
...
| คำสั่ง | คำอธิบายแบบมีหลักการ | ตัวอย่าง |
|--------|------------------------|-----------|
| `-f` | fragment packet เพื่อหลบ firewall ที่ไม่สามารถ reassemble ได้ | `nmap -f 192.168.1.5` |
| `-D` | ใช้ decoy IP เพื่อสร้างความสับสนให้กับ IDS | `nmap -D RND:10 192.168.1.5` |
| `-S` | ปลอมแปลง source IP ของ packet | `nmap -S 1.2.3.4 192.168.1.5` |
| `--spoof-mac` | ปลอม MAC address เพื่อหลีกเลี่ยงการ track | `nmap --spoof-mac 00:11:22:33:44:55 192.168.1.5` |
...
| คำสั่ง | คำอธิบายแบบมีหลักการ | ตัวอย่าง |
|--------|------------------------|-----------|
| `-oN file.txt` | บันทึกผลลัพธ์แบบ text ปกติ | `nmap -oN result.txt 192.168.1.5` |
| `-oX file.xml` | บันทึกผลแบบ XML เพื่อใช้งานกับ script หรือแปลงผล | `nmap -oX result.xml 192.168.1.5` |
| `-oG file.grep` | Grepable output ใช้กับ script ต่อได้ง่าย | `nmap -oG result.grep 192.168.1.5` |
| `-oA basename` | บันทึกผลทั้ง 3 รูปแบบ | `nmap -oA scan 192.168.1.5` |
